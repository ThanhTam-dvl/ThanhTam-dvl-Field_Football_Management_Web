// ====== frontend/src/admin/components/maintenance/MaintenanceStats.jsx ======
const MaintenanceStats = ({ stats }) => {
  const statItems = [
    ...
    },
    {
      ...
      trend: stats.active_maintenance > 0 ? 'Có việc' : 'Rảnh',
      trendUp: stats.active_maintenance === 0
    },
    {
      key: 'upcoming',
    ...
    },
    {
      ...
      value: stats.overdue_maintenance || 0,
      label: 'Quá hạn',
      color: 'from-red-500 to-red-600',
      bgColor: 'bg-red-50 dark:bg-red-900/20',
      iconColor: 'text-red-600 dark:text-red-400',
      trend: stats.overdue_maintenance > 0 ? 'Cần xử lý' : 'Tốt',
      trendUp: stats.overdue_maintenance === 0
    },
    {
      key: 'completed',
      icon: 'fas fa-check-circle',
      value: stats.completed_maintenance || 0,
      ...
      {statItems.map((item, index) => (
        ...
        >
          ...
          <div className={`absolute top-2 right-2 w-2 h-2 ${
            item.key === 'overdue' && stats.overdue_maintenance > 0 
              ? 'bg-red-500 animate-pulse' 
              : item.key === 'active' && stats.active_maintenance > 0
                ? 'bg-amber-500 animate-pulse'
                : item.key === 'upcoming' && stats.upcoming_maintenance > 0
